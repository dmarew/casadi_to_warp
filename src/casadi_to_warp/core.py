...
class CasadiToWarp:
    ...
    """
    def __init__(self, casadi_func: ca.Function, function_name="casadi_kernel_gen", output_dir="."):
        ...
        self.math_map = {
            r'\bcasadi_sqrt\(': 'safe_sqrt(', r'\bsqrt\(': 'safe_sqrt(',
            r'\basin\(': 'safe_asin(',        r'\bacos\(': 'safe_acos(',
            ...
            r'\batan\(': 'wp.atan(', r'\batan2\(': 'wp.atan2(',
    ...
    def _parse_body(self, c_code):
        ...
        for line in lines:
            ...
            if '?' in line and ':' in line:
                def ternary_replacer(m):
                    ...
                    if cond.startswith('(') and false_val.endswith(')'):
                        if cond.count('(') > cond.count(')') and false_val.count(')') > false_val.count('('):
                            cond = cond[1:].strip()
                            false_val = false_val[:-1].strip()

                    if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', cond): cond = f"({cond} != 0.0)"
                    elif cond.startswith('(') and cond.endswith(')'):
                         inner = cond[1:-1]
                         if re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', inner): cond = f"({cond} != 0.0)"
                    return f"= {true_val} if ({cond}) else {false_val};"
                
                line = re.sub(r'=\s*(.*?)\s*\?\s*(.*?)\s*:\s*(.*?);', ternary_replacer, line)
            ...
            def index_replacer(m):
                ...
                if "inputs" in name: return f"FloatT({ref})"
                return ref
            ...
            if line.strip().startswith("outputs_"):
                ...
                if has_semicolon: rhs = rhs[:-1]
                
                # Cleanup lingering closing paren if any
                if rhs.endswith(")") and "(" not in rhs: rhs = rhs[:-1]
    ...
    def transpile(self):
        ...
        python_src.append(f"{indent}return wp.acos(wp.clamp(x, FloatT(-0.9999999), FloatT(0.9999999)))")
        python_src.append("")
        
        python_src.append("@wp.func")
        python_src.append("def safe_asin(x: FloatT):")
        python_src.append(f"{indent}return wp.asin(wp.clamp(x, FloatT(-0.9999999), FloatT(0.9999999)))")
        python_src.append("")
        
        python_src.append("@wp.func")
        python_src.append("def c_copysign(x: FloatT, y: FloatT):")
        python_src.append(f"{indent}return wp.abs(x) if (y >= FloatT(0.0)) else -wp.abs(x)")
        python_src.append("")
